...
- **Sheet viewer tabs** ✓ — **V1 Sheet** (read-only) and **V2.1 Sheet** show the uploaded workbook *as is* (all original columns, raw cell text). Every column has a searchable multi-select filter plus global search, column show/hide, and pagination (scales to ~2,000 rows). The **V2.1 Sheet** additionally lets you **edit any cell inline** (changed cells highlighted, edit counter, reset) and **Download Excel** to get the edited `.xlsx`.
