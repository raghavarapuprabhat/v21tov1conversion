...
- **MoM export** ✓ — the landing **Export MoM** button produces stakeholder-ready Minutes of Meeting for a **From/To date range** (presets: Today / Last 7 days / This month): all status **decisions** and **comments** made in that window, grouped by attribute (IS), with a decisions table and a per-attribute discussion log. Live preview of counts/participants; downloads as styled **HTML** (print/email), **Markdown**, or **CSV** (`/api/export/mom` + `/api/export/mom.json`). Days are matched in local time.
