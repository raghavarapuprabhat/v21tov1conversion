...
- **E3 gap engines + severity** ✓ — G1 coverage funnel (A/B/C), G2 occurrence (per context, arrays), G3 data type (type-equivalence map), G4 mandatory/optional, G5 reverse-orphan; position-independent `gap_id`, severity, summary, analysis orchestrator. On the sample: G1 A=4/B=3/C=3, G5=1.
...
- **E8 REST API** ✓ — `/summary`, `/gaps` (per-column + global filter, sort, paginate), `/gaps/{id}` + `/comments` + `/history`, `PATCH status` & `bulk-status`, `POST comments`, `/tree`, `/v2/by-dd/{dd}`, `/views`, `/export` (CSV). 55 tests pass; OpenAPI at `/docs`.
- **E9–E13 React UI** ✓ — white Tailwind theme: landing gap cards (G1 A/B/C funnel), gap data grid with **per-column filters** — **multi-select for IS and Path** (searchable/scrollable, scales to ~800 values via a `/facets` endpoint), dropdowns for Severity/Context/Status, text for V1/V2/Detail/DD — plus sort, column show/hide, CSV export, bulk disposition. New **Path column** (Level 1–8 joined, truncated with hover tooltip). Conversation panel (threaded comments, status + history, Fetch-V2-By-DD, "earlier discussion for IS"); V1 tree with gap rollups. Display-name prompt; `?as=` and `?open=` deep links.
